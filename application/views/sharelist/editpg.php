...
if(isset($dataList['oldTeacher'])){
	$hidden['oldTeacher'] = $dataList['oldTeacher'];
}
echo form_open_multipart('sharelist/setShareData', $attributes,$hidden);
?>
			...
			<td style="background-color:#E9F0DE;">
				<div class="mode_item">
					<table>
						<tr>
							<td>
								<p>題目標題：<input type="text" class="put_2" name="title_dsc" id="title_dsc" value="<?php echo isset($dataList['title_dsc'])?$dataList['title_dsc']:'';?>"></p>
							</td>
						</tr>
						<tr>
							<td>
								<p>題目註解：<textarea name="remark_dsc" class="tex_2" id="remark_dsc" ><?php echo isset($dataList['remark_dsc'])?$dataList['remark_dsc']:'';?></textarea></p>
							</td>
						</tr>
						<tr>	
							<td>
								<p>題目類型：<select name="is_practice" class="put_2" id="is_practice" >
				...
				?>	
						...
						<tr>	
							<td>
								<p>測驗起始日期：<input type="text" class="datepicker" name="begin_date" id="begin_date" value="<?php echo isset($dataList['begin_date'])?$dataList['begin_date']:'';?>"></p>
							</td>
						</tr>
						<tr>	
							<td>						
								<p>測驗結束日期：<input type="text" class="datepicker"  name="end_date" id="end_date" value="<?php echo isset($dataList['end_date'])?$dataList['end_date']:'';?>"></p>
							</td>
						</tr>
						<tr>	
							<td>
								<span><a class="button" title="下一步驟" onclick="goNext()">下一步驟</a></span>
							</td>
						</tr>
					</table>
				</div>	
		...
		<tr id="tr_1" style="display:none;">
			...
			<table>
				<tr class="title">
					<td>模組1 閱讀出題模組</td>
					<td>
						<input type="radio" name="m1" id="m1_null" value="" checked>
						<label for="m1_null">不選用</label>					
<?php 
...
?>									
				...
				<tr class="title">
					<td>模組2 搶25遊戲模組</td>
					<td>
						<input type="radio" name="m2" id="m2_null" value="" checked>
						<label for="m2_null">不選用</label>
<?php 
...
?>
					...
					<td>
						<input type="radio" name="m2" id="m2_<?php echo $key;?>" value="<?php echo $tempArray['num'];?>"  <?php echo $dsc;?>>
						<label for="m2_<?php echo $key;?>"><?php echo $tempArray['title_dsc'];?></label>
<?php		
	}
}
?>						
				...
				<tr class="title">
					<td>模組3 思樂冰製作遊戲模組</td>
					<td>	
						<input type="radio" name="m3" id="m3_null" value="" checked>
						<label for="m3_null">不選用</label>
...
?>					</td>
					<td>
						<input type="radio" name="m3" id="m3_<?php echo $key;?>" value="<?php echo $tempArray['num'];?>"  <?php echo $dsc;?>>
						<label for="m3_<?php echo $key;?>"><?php echo $tempArray['title_dsc'];?></label>
<?php		
	}
}
?>						
				...
				<tr class="title">
					...
					<td>
						<input type="radio" name="m4" id="m4_null" value="" checked>
						<label for="m4_null">不選用</label>
...
?>
					...
					<td>
						<input type="radio" name="m4" id="m4_<?php echo $key;?>" value="<?php echo $tempArray['num'];?>"  <?php echo $dsc;?>>
						<label for="m4_<?php echo $key;?>"><?php echo $tempArray['title_dsc'];?></label>
<?php		
	}
}
?>						
				...
				<td>
					<input type="radio" name="m5" id="m5_null" value="" checked>
					<label for="m5_null">不選用</label>
<?php 
...
?>
				...
				<td>
					<input type="radio" name="m5" id="m5_<?php echo $key;?>" value="<?php echo $tempArray['num'];?>" <?php echo $dsc;?>>
					<label for="m5_<?php echo $key;?>"><?php echo $tempArray['title_dsc'];?></label>
<?php		
	}
}
?>					
			...
			<table width="100%">
				...
				<tr>
					...
					<div class="mode_item">
						<ul>
<?php
if(isset($schoolData) and count($schoolData) > 0){
	foreach($schoolData as $key => $value){
?>
	<li onclick="swSchool('<?php echo $key; ?>')"><?php echo $value; ?></li>
<?php		
	}
}
?>					
					...
					<ul id='swTeacherArea' >
<?php
if(isset($dataList['swTeacher']) and count($dataList['swTeacher']) > 0){
	foreach($dataList['swTeacher'] as $key => $teacherNum){
?>
<li id="li_<?php echo $key;?>" ><?php echo isset($teacherNameData[$teacherNum])?$teacherNameData[$teacherNum]:'';?><input type="hidden" name="shareTeacher[]" value="<?php echo $teacherNum;?>"><input type="button" onclick="$('#li_<?php echo $key;?>').remove();" value="移除"></li>
<?php
	$li_index++;
	}
}
?>					
	...
	<tr id="tr_4" style="display:none;">
		...
		<div class="mode_item">
		 <h2>語意分析字詞設定<h2>
		 <button type="button" onclick="add_analysis()">新增字詞</button>
		<ul id="analysis_area">
<?php
...
var teacher_list = [];//教師列表
var obj = {'schoolNum':'','num':'','name':''};
<?php
if(isset($teacherData) and count($teacherData) > 0){
	foreach($teacherData as $key => $tempArray){
		if(is_array($tempArray)){
			foreach($tempArray as $tArray){
?>
	obj = {'schoolNum':'<?php echo $tArray['schoolNum'];?>','num':'<?php echo $tArray['num'];?>','name':'<?php echo $tArray['c_name'];?>'};
	teacher_list.push(obj);
<?php		
			}			
		}
	}	
...
//顯示指定學校內的所有老師資料
function swSchool(getID){
	$('#teacherArea').html('');
	var htmlDsc = '<ul>';
	for(var x=0;x<teacher_list.length;x++){
		if(teacher_list[x]['schoolNum'] == getID){
			var tDsc = '<li>'+ teacher_list[x]['name']  +'<input type="button" onclick="addTeacher(\''+ x  +'\')" value="新增"></li>';
			htmlDsc = htmlDsc + tDsc;
		}
	}
	htmlDsc = htmlDsc + '</ul>';
	$('#teacherArea').append(htmlDsc);
}

//選擇要分享的老師
var li_index = <?php echo isset($li_index)?$li_index:'0';?>;
function addTeacher(getIndex){
	var tDsc = '<li id="li_'+li_index+'" >'+ teacher_list[getIndex]['name']  +'<input type="hidden" name="shareTeacher[]" value="'+teacher_list[getIndex]['num']+'"><input type="button" onclick="$(\'#li_'+li_index+'\').remove();" value="移除"></li>';
	$('#swTeacherArea').append(tDsc);
	li_index++;
}
</script>